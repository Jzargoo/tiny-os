...
use crate::{acpi::madt_entries::EntryType, hal::addresses::PhysicalAddress, println};
...
impl<'a, P: PhysicalAddress> Iterator for MadtIterator<'a, P> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if next_offset > self.data_slice.len() {
            return None;
        }

        let payload =
            &self.data_slice[self.curr_offset + 2..next_offset];
